...
namespace PRN231_API.Controllers
{
    ...
    public class ComicsController : ControllerBase
    {
        ...
        [HttpGet("popular")]
        public async Task<ActionResult<IEnumerable<Comic>>> GetPopular()
        {
        ...
        [HttpGet("sameAuthor/{userId}")]
        public async Task<ActionResult<List<Comic>>> GetSameAuthor(int userId)
        {
            var comics = await context.Comics.Include(c => c.User)
        ...
        public async Task<ActionResult<IEnumerable<Comic>>> GetByName(string name)
        {
            var comics = await context.Comics
                .Where(c => c.ComicName.ToLower().Trim().Contains(name.ToLower().Trim()))
                .ToListAsync();
            return Ok(comics);
        }
        [HttpGet("totalPage")]
